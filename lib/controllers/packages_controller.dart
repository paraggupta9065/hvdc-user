...
class PackagesController extends GetxController {
  ...
  List<Packages> packagesSearch = [];

  Future getSearch({required String key}) async {
    try {
      isLoadingSearch.value = true;
      String endpoint = "/packages/?key=$key";
      ...
      List rawPackages = response['results'];
      packagesSearch.clear();
      packagesSearch = List<Packages>.generate(
          rawPackages.length, (index) => Packages.fromJson(rawPackages[index]));
    } catch (e) {
      kShowSnackbar(title: "Error !", message: e.toString());
    } finally {
